...
evalElcNu(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  fout[0] = nu/(1 + exp(fabs(x)/(3.0*lambda_D) - 8.0/1.5));
}
...
create_ctx(void)
{
  ...
  double nu_ee = vte/(5.0*lambda_D);
  double nu_ii = vti/(5.0*lambda_D);
...
main(int argc, char **argv)
{
  ...
  struct gkyl_bc_emission_ctx *bc_ctx = gkyl_bc_emission_new(ctx.num_emission_species,
    0.0, false, spectrum_model, yield_model, NULL, in_species);
