...
struct gk_app_ctx create_ctx(void)
{
  ...
  double kappa     = 1.395113;               // Elongation (=1 for no elongation).
  double delta     = 0.282550;               // Triangularity (=0 for no triangularity).
  ...
  // Reference values.
  double Te0 = 100*GKYL_ELEMENTARY_CHARGE;
  double Ti0 = 100*GKYL_ELEMENTARY_CHARGE;
  ...
  double nrhos_y = 150; // Number of rho_s in the y-direction, used to set the domain size along y.
  double nvth_vpar_e = 6.0; // sets vpar max e in terms of vthe.
  double nvth_vpar_i = 6.0; // sets vpar max i in terms of vthi.
  double nvth_mu_e = 1.5; // sets mu max e in terms of mi*pow(4*vti,2)/(2*B0).
  double nvth_mu_i = 1.5; // sets mu max i in terms of mi*pow(4*vti,2)/(2*B0).
  ...
  double Ly        = nrhos_y*rho_s;           // Domain size along y.
  ...
  double vpar_max_elc = nvth_vpar_e*vte;
  double mu_max_elc   = nvth_mu_e*me*pow(4*vte,2)/(2*B0);
  double vpar_max_ion = nvth_vpar_i*vti;
  double mu_max_ion   = nvth_mu_i*mi*pow(4*vti,2)/(2*B0);
