...
void
eval_temp_ar0(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = T;
...
void
eval_temp_ar1(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = T;
...
void
eval_temp_ar2(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 2.5*T;
...
void
eval_temp_ar3(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 5.0*T;
...
void
eval_temp_ar4(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 12.0*T;
...
void
eval_temp_ar5(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 20.0*T;
...
void
eval_temp_ar6(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 25.0*T;
...
void
eval_temp_ar7(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 35.0*T;
...
void
eval_temp_ar8(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_step_ctx *app = ctx;
  double T = app->TAr;
  fout[0] = 45.0*T;
...
create_ctx(void)
{
  ...
  double t_end = 12.0e-3; 
  double num_frames = 120 * 10;
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_species elc = {
    ...
    .collisions =  {
      ...
      .cross_nu_fac = {
        ...
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massElc, ctx.massAr, ctx.chargeElc, 5*ctx.chargeIon, ctx.Te, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massElc, ctx.massAr, ctx.chargeElc, 6*ctx.chargeIon, ctx.Te, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massElc, ctx.massAr, ctx.chargeElc, 7*ctx.chargeIon, ctx.Te, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massElc, ctx.massAr, ctx.chargeElc, 8*ctx.chargeIon, ctx.Te, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuElc,
      .num_cross_collisions = 9,
      .collide_with = { "ion", "Ar1", "Ar2", "Ar3", "Ar4", "Ar5", "Ar6", "Ar7", "Ar8" },
    },
 
    .radiation = {
        .radiation_id = GKYL_GK_RADIATION,
        .num_cross_collisions = 9,
        .collide_with = { "Ar0", "Ar1", "Ar2", "Ar3", "Ar4", "Ar5", "Ar6", "Ar7", "Ar8"},
        .z = {18, 18, 18, 18, 18, 18, 18, 18, 18},
        .charge_state = { 0, 1, 2, 3, 4, 5, 6, 7, 8},
        .num_of_densities = {1, 1, 1, 1, 1, 1, 1, 1, 1},
    },
  ...
  struct gkyl_gyrokinetic_species ion = {
    ...
    .collisions =  {
      ...
      .cross_nu_fac = {
        ...
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massIon, ctx.massAr, ctx.chargeIon, 5*ctx.chargeIon, ctx.Ti, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massIon, ctx.massAr, ctx.chargeIon, 6*ctx.chargeIon, ctx.Ti, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massIon, ctx.massAr, ctx.chargeIon, 7*ctx.chargeIon, ctx.Ti, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0, ctx.n0Ar, ctx.massIon, ctx.massAr, ctx.chargeIon, 8*ctx.chargeIon, ctx.Ti, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "Ar1", "Ar2", "Ar3", "Ar4" , "Ar5", "Ar6", "Ar7", "Ar8"},
    },
  ...
  struct gkyl_gyrokinetic_species Ar1 = {
    ...
    .collisions =  {
      ...
      .cross_nu_fac = {
        ...
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar2", "Ar3", "Ar4" , "Ar5", "Ar6", "Ar7", "Ar8"},
    },
  ...
  struct gkyl_gyrokinetic_species Ar2 = {
    ...
    .collisions =  {
      ...
      .cross_nu_fac = {
        ...
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 2*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 2*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 2*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 2*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar3", "Ar4" , "Ar5", "Ar6", "Ar7", "Ar8"},
    },
  ...
  struct gkyl_gyrokinetic_species Ar3 = {
    ...
    .upper = { 3*ctx.vpar_max_Ar, 2*ctx.mu_max_Ar}, 
    .cells = { NV, NMU },
    ...
    .collisions =  {
      ...
      .cross_nu_fac = {
        ...
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 3*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 3*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 3*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 3*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar2", "Ar4" , "Ar5", "Ar6", "Ar7", "Ar8"},
    },
  ...
  struct gkyl_gyrokinetic_species Ar4 = {
    ...
    .lower = { -5.5*ctx.vpar_max_Ar, 0.0},
    .upper = { 5.5*ctx.vpar_max_Ar, 4*ctx.mu_max_Ar}, 
    .cells = { NV, NMU },
    ...
    .collisions =  {
      ...
      .cross_nu_fac = {
        ...
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 4*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 4*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 4*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 4*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar2", "Ar3" , "Ar5", "Ar6", "Ar7", "Ar8"},
    },
  ...
  struct gkyl_gyrokinetic_species Ar5 = {
    ...
    .upper = { 6*ctx.vpar_max_Ar, 4*ctx.mu_max_Ar}, 
    .cells = { NV*10, NMU },
    .polarization_density = ctx.n0Ar*32.0,
    .enforce_positivity=true,
    ...
    .collisions =  {
      .collision_id = GKYL_LBO_COLLISIONS,
      .normNu = true,
      .self_nu_fac = norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
      .cross_nu_fac = {
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massElc, 5*ctx.chargeIon, ctx.chargeElc, ctx.TAr, ctx.Te), 
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massIon, 5*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.Ti),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 2*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 3*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 4*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 5*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar2", "Ar3" , "Ar4", "Ar6", "Ar7", "Ar8"},
    },
  ...
  struct gkyl_gyrokinetic_species Ar6 = {
    ...
    .lower = { -8.5*ctx.vpar_max_Ar, 0.0},
    .upper = { 8.5*ctx.vpar_max_Ar, 8*ctx.mu_max_Ar}, 
    .cells = { NV, NMU },
    .polarization_density = ctx.n0Ar*64,
    .enforce_positivity=true,
    ...
    .collisions =  {
      .collision_id = GKYL_LBO_COLLISIONS,
      .normNu = true,
      .self_nu_fac = norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
      .cross_nu_fac = {
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massElc, 6*ctx.chargeIon, ctx.chargeElc, ctx.TAr, ctx.Te), 
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massIon, 6*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.Ti),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 2*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 3*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 4*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 6*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar2", "Ar3", "Ar4", "Ar5", "Ar7", "Ar8" },
    },
  ...
  struct gkyl_gyrokinetic_species Ar7 = {
    ...
    .lower = { -9*ctx.vpar_max_Ar, 0.0},
    .upper = { 9*ctx.vpar_max_Ar, 8*ctx.mu_max_Ar}, 
    .cells = { NV, NMU },
    .polarization_density = ctx.n0Ar*128,
    .enforce_positivity=true,
    ...
    .collisions =  {
      .collision_id = GKYL_LBO_COLLISIONS,
      .normNu = true,
      .self_nu_fac = norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr),
      .cross_nu_fac = {
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massElc, 7*ctx.chargeIon, ctx.chargeElc, ctx.TAr, ctx.Te), 
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massIon, 7*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.Ti),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 2*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 3*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 4*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 7*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar2", "Ar3", "Ar4","Ar5", "Ar6", "Ar8" },
    },
  ...
  struct gkyl_gyrokinetic_species Ar8 = {
    ...
    .upper = { 9*ctx.vpar_max_Ar, 8*ctx.mu_max_Ar}, 
    .cells = { NV, NMU },
    .polarization_density = ctx.n0Ar*256,
    .enforce_positivity=true,
    ...
    .collisions =  {
      .collision_id = GKYL_LBO_COLLISIONS,
      .normNu = true,
      .self_nu_fac = norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 8*ctx.chargeIon, ctx.TAr, ctx.TAr),
      .cross_nu_fac = {
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massElc, 8*ctx.chargeIon, ctx.chargeElc, ctx.TAr, ctx.Te), 
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0, ctx.massAr, ctx.massIon, 8*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.Ti),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 2*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 3*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 4*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 5*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 6*ctx.chargeIon, ctx.TAr, ctx.TAr),
        norm_nu_func(ctx.nuFrac, ctx.n0Ar, ctx.n0Ar, ctx.massAr, ctx.massAr, 8*ctx.chargeIon, 7*ctx.chargeIon, ctx.TAr, ctx.TAr)
      },
      .bmag_mid = 2.51,
      .ctx = &ctx,
      .self_nu = evalNuIon,
      .num_cross_collisions = 9,
      .collide_with = { "elc", "ion", "Ar1", "Ar2", "Ar3", "Ar4", "Ar5", "Ar6", "Ar7" },
    },
  ...
  struct gkyl_gk gk = {
    ...
    .num_species = 10,
    .species = { elc, ion, Ar1, Ar2, Ar3, Ar4, Ar5, Ar6, Ar7, Ar8},
    .num_neut_species = 1,
    .neut_species = {Ar0},
