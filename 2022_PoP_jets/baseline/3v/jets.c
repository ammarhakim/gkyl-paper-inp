...
evalDistFuncElc(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double fv = 0.0;
  double norm = sqrt(2.0*M_PI*sq(vt)); 
  if (x < 0)
    fv = n0/(norm*norm*norm) * exp(-sq(vx-vdrift)/(2*sq(vt))) * exp(-sq(vy)/(2*sq(vt))) * exp(-sq(vz)/(2*sq(vt)));
  else
    fv = n0/(norm*norm*norm) * exp(-sq(vx+vdrift)/(2*sq(vt))) * exp(-sq(vy)/(2*sq(vt))) * exp(-sq(vz)/(2*sq(vt)));
  fout[0] = fv;
}
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species elc = {
    ...
    .num_diag_moments = 5,
    .diag_moments = { "M0", "M1i", "M2", "M2ij", "M3i" },
  };
  ...
  struct gkyl_vlasov_species ion = {
    ...
    .lower = { -3 * ctx.uShock, -3 * ctx.uShock, -3 * ctx.uShock},
    .upper = { 3 * ctx.uShock, 3 * ctx.uShock, 3 * ctx.uShock}, 
    .cells = { 16, 16, 16 },
  ...
  double tcurr = 0.0, tend = 1e-7;
  double dt = tend-tcurr;
  int nframes = 20, frame_count = 1;
