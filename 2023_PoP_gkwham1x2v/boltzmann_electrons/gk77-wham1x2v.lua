...
local function Z_psiz(psiIn,zIn)
   ...
   local Zout = 0.0
   local eps  = macL/numCellZ -- Interestingly using a smaller eps yields larger errors in some geo quantities.
   if diff.le(0.,zIn) then
...
-- Functions to penalize the mu*B term in the Hamiltonian in order to weaken
-- the mirror force in the inaccessible region and relax the CFL constraint there.
local muMax_i = mi*((3.*vti)^2)/(2*B_p)
local muBMax0_i = muMax_i*B_p

local function chi_zmu(zIn, muIn)
   -- The factor chi(z,mu) multiplies the mirror force term.
   local Z = Z_psiz(fldLinePsi,zIn)         -- Cylindrical axial coordinate.
   local _, _, Bmag = Bfield_psiZ(fldLinePsi,Z)
   local mu_thresh = muBMax0_i/Bmag
   local width = 2.*(1./20.)*mu_thresh
   local chi = 0.5*(1.+math.tanh((mu_thresh+2.5*width - muIn)/(0.5*width)))
   return chi
end

local function func_chi(t, xn)
   -- The factor chi(z,mu) multiplies the mirror force term.
   local z, vpar, mu = xn[1], xn[2], xn[3]
   return chi_zmu(z,mu)
end

local function dBdz(psiIn,zIn)
   local function Bmag_z(z)
      local Z = Z_psiz(psiIn,z)         -- Cylindrical axial coordinate.
      local _, _, Bmag = Bfield_psiZ(psiIn,Z)
      return Bmag
   end
   local dB = df(Bmag_z,1)(zIn)
   return dB
end

local function func_Pi(t, xn)
   -- Pi(z,mu) factor that will get projected onto the basis
   -- and will multiply the mu*Pi factor in the Hamiltonian.
   local z, vpar, mu = xn[1], xn[2], xn[3]

   local function integrand(t)
      local Z = Z_psiz(fldLinePsi,t)         -- Cylindrical axial coordinate.
      local BmagP = dBdz(fldLinePsi,t)
      local chi = chi_zmu(t,mu)
      return chi*BmagP
   end
   log(string.format("z = %g | vpar = %g | mu = %g\n",xn[1], xn[2], xn[3]))
   local integral = 0.
   if diff.lt(z, 0.) then
      local intV, _ = quad.dblexp(integrand, z, 0., 1e-9)
      integral = -intV
   elseif diff.lt(0., z) then
      integral, _ = quad.dblexp(integrand, 0., z, 1e-9)
   end
   log(string.format("z = %g | vpar = %g | mu = %g\n",xn[1], xn[2], xn[3]))
   local Z = Z_psiz(fldLinePsi,z)         -- Cylindrical axial coordinate.
   local _, _, Bmag = Bfield_psiZ(fldLinePsi,Z)
   return (B_p+integral)/Bmag
end

local function func_Pi2chi(t, xn)
   local z, vpar, mu = xn[1], xn[2], xn[3]
   local function BmagPi(zIn)
      local Z = Z_psiz(fldLinePsi,zIn)         -- Cylindrical axial coordinate.
      local _, _, Bmag = Bfield_psiZ(fldLinePsi,Z)
      local Pi = func_Pi(t, {zIn, vpar, mu})
      return Bmag*Pi
   end
   local dfdz = df(BmagPi,1)(z)
   local chi = dfdz/dBdz(fldLinePsi,z)
   return chi
end
...
--      mirrorPenalty = func_chi,
      mirrorPenalty = "ion_mirrorPenalty_0.bp",
