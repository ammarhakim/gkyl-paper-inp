...
local muBMax0_i, muBMax0_e = muMax_i*B_p, muMax_e*B_p

local function phiApprox(zIn)
   -- Approximate electrostatic potential. Based on previous simulations or Pastukhov potential.
   if math.abs(zIn) <= z_m then
      local A, sig, p = 10560., 0.98, 3
      return A*math.exp(-0.5*(math.abs(zIn)/sig)^p)
   elseif zIn < -z_m then
      local A, mu, p = 10560*0.69, -0.11, 1.5
      return A/((-zIn-mu)^p)
   elseif zIn > z_m then
      local A, mu, p = 10560*0.69, -0.11, 1.5
      return A/(( zIn-mu)^p)
...
local phiApprox_0 = phiApprox(0)

local function chi_zmu_i(zIn, muIn)
   -- The factor chi(z,mu) multiplies the mirror force term.
   local Z = Z_psiz(fldLinePsi,zIn)         -- Cylindrical axial coordinate.
   local _, _, Bmag = Bfield_psiZ(fldLinePsi,Z)
   local DeltaPhi = 0. --phiApprox_0 - phiApprox(zIn)
   local mu_thresh = (muBMax0_i+qi*DeltaPhi)/Bmag
   local width = 2.*(1./20.)*mu_thresh
   local chi = 0.5*(1.+math.tanh((mu_thresh+2.5*width - muIn)/(0.5*width)))
   return chi
end

local function chi_zmu_e(zIn, muIn)
   -- The factor chi(z,mu) multiplies the mirror force term.
   local Z = Z_psiz(fldLinePsi,zIn)         -- Cylindrical axial coordinate.
   local _, _, Bmag = Bfield_psiZ(fldLinePsi,Z)
   local mu_thresh = muBMax0_e/Bmag
--   local DeltaPhi = phiApprox_0 - phiApprox(zIn)
--   if math.abs(zIn) < z_m then
--      mu_thresh = mu_thresh + 0.5*(qe*DeltaPhi)/Bmag
--   else
--      mu_thresh = mu_thresh + (qe*DeltaPhi)/Bmag
--   end
   local width = 2.*(1./20.)*mu_thresh
   local chi = 0.5*(1.+math.tanh((mu_thresh+2.5*width - muIn)/(0.5*width)))
--   if muIn < math.abs(mu_thresh) then return 1.0 else return 0. end
   return chi
end

local function func_chi_i(t, xn)
   -- The factor chi(z,mu) multiplies the mirror force term.
   local z, vpar, mu = xn[1], xn[2], xn[3]
   return chi_zmu_i(z,mu)
end

local function func_chi_e(t, xn)
   -- The factor chi(z,mu) multiplies the mirror force term.
   local z, vpar, mu = xn[1], xn[2], xn[3]
   return chi_zmu_e(z,mu)
end
...
--      mirrorPenalty = func_chi_e,
      mirrorPenalty = "elc_mirrorPenalty_0.bp",
