...
first_job_id=$(sbatch jobscript_gkyl-stellar-amd | awk '{print $4}')
...
do
  ...
  next_job_id=$(sbatch --dependency=afterany:$previous_job_id jobscript_gkyl-stellar-amd | awk '{print $4}')
  job_ids+=($next_job_id)
done
