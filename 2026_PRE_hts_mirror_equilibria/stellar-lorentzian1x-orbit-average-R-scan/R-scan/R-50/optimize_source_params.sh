...
AMP_LINE_NUM=""
TEMP_LINE_NUM=""
AMP_LINE_MATCH='^[[:space:]]*double[[:space:]]+ion_source_amplitude[[:space:]]*='
TEMP_LINE_MATCH='^[[:space:]]*double[[:space:]]+ion_source_temp[[:space:]]*='
AMP_LINE_FORMAT='double ion_source_amplitude = %s; // Beam intM0 = %s'
TEMP_LINE_FORMAT='double ion_source_temp = %s * eV ; // Beam intM2 = %s'
...
get_source_line() {
  ...
  case "$kind" in
    amp)
      ...
      label="ion_source_amplitude"
      ;;
    temp)
      line_num="$TEMP_LINE_NUM"
      match="$TEMP_LINE_MATCH"
      label="ion_source_temp"
      ;;
    *)
