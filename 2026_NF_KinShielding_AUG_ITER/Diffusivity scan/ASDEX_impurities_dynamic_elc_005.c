...
void
eval_udrift_H0(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  fout[0] = 0.0; 
  fout[1] = 0.0;
  fout[2] = 0.0;
...
void
eval_density_imp0(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_asdex_ctx *app = ctx;
  double x = xn[0], z = xn[1];
  double n0 = app->n0imp;
...
void
eval_temp_imp0(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_asdex_ctx *app = ctx;
  double T = app->Timp0;
  fout[0] = T;
...
void
diffusion_D_imp_func(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct gk_asdex_ctx *app = ctx;

  fout[0] = app->diffusionD_imp; // Diffusivity [m^2/s].
}
...
void mapc2p_vel_imp(double t, const double *vc, double* GKYL_RESTRICT vp, void *ctx)
{
  struct gk_asdex_ctx *app = ctx;
  double vpar_max_imp = app->vpar_max_imp;
  double mu_max_imp = app->mu_max_imp;

  double cvpar = vc[0], cmu = vc[1];
  // Linear map up to vpar_max/2, then quadratic.
  if (fabs(cvpar) <= 0.5)
    vp[0] = vpar_max_imp*cvpar;
  else if (cvpar < -0.5)
    vp[0] = -vpar_max_imp*2.0*pow(cvpar,2);
  else
    vp[0] =  vpar_max_imp*2.0*pow(cvpar,2);

  // Quadratic map in mu.
  vp[1] = mu_max_imp*pow(cmu,2);
}
...
create_ctx(void)
{
  ...
  double mimp = 39.95*GKYL_PROTON_MASS;
  double n0imp = 1e13;
  double Timp0 = 10*eV;
  double cz = 0.68;
  ...
  double diffusionD = 0.2; // Particle diffusion coefficient.
  double diffusionD_imp = 0.05;
  ...
  double ndot_src = 2.0*2.0*0.7e19*c_s/(Lc_src/4.0);
  double Te_src = 1.3*1.5*Te;
  double Ti_src = 1.3*1.5*Ti;
  ...
  double vpar_max_imp = 6.0*vtImp;
  double mu_max_imp = mimp*pow(4.0*vtImp,2)/(2.0*B0);
  ...
  struct gk_asdex_ctx ctx = {
	...
	.mimp = mimp,
	.vtImp = vtImp,
	.vpar_max_imp = vpar_max_imp,
	.mu_max_imp = mu_max_imp,
  };
  return ctx;
}
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gyrokinetic_species elc = {
    ...
    .collisions =  {
      ...
      .collide_with = { "ion", "imp1", "imp2", "imp3", "imp4" },
    },
     ...
     .react_neut = {
      ...
      .react_type = {
        { .react_id = GKYL_REACT_IZ,
          ...
          .ion_id = GKYL_ION_AR,
          .elc_nm = "elc",
          .ion_nm = "imp1", // ion is always the higher charge state
          .donor_nm = "imp0", // interacts with elc to give up charge
    ...
    .react= { 
      ...
      .react_type = { 
          ...
          .ion_id = GKYL_ION_AR, 
          .elc_nm = "elc", 
          .ion_nm = "imp2", // ion is always the higher charge state  
          .donor_nm = "imp1", // interacts with elc to give up charge 
...
//      .write_diagnostics = true,
    ...
    .num_diag_moments = 5,
    .diag_moments = { GKYL_F_MOMENT_M1,  GKYL_F_MOMENT_M2PAR, GKYL_F_MOMENT_M2PERP, GKYL_F_MOMENT_BIMAXWELLIAN, GKYL_F_MOMENT_MAXWELLIAN},
...
//      .time_integrated = true,
  ...
  struct gkyl_gyrokinetic_species ion = {
    ...
    .collisions =  {
      ...
      .num_cross_collisions = 5,
      .collide_with = { "elc", "imp1", "imp2", "imp3", "imp4"},
    },
...
//      .time_integrated = true,
  ...
  struct gkyl_gyrokinetic_neut_species imp0 = {
    .name = "imp0", .mass = ctx.mimp,
    .vdim = 0,
    ...
    .projection = {
      .proj_id = GKYL_PROJ_MAXWELLIAN_PRIM, 
      .ctx_density = &ctx,
      .density = eval_density_imp0,
      .ctx_upar = &ctx,
      .udrift= eval_udrift_H0,
      .ctx_temp = &ctx,
      .temp = eval_temp_imp0,      
    },
...
struct gkyl_gyrokinetic_species imp1 = {
    .name = "imp1",
    .vdim = ctx.vdim,
    .charge = ctx.charge_ion, .mass = ctx.mimp,
    .lower = { ctx.vpar_min_ion_c, ctx.mu_min_ion_c},
    .upper = { ctx.vpar_max_ion_c, ctx.mu_max_ion_c}, 
    .cells = { cells_v[0], cells_v[1] },
    .polarization_density = ctx.n0imp,
    //.no_by = true, 
    
    .mapc2p = {
      .mapping = mapc2p_vel_imp,
      .ctx = &ctx
    },
    ...
    .collisions =  {
      ...
      //.self_nu = evalNuIon,
      .num_cross_collisions = 5,
      .collide_with = {"elc", "ion", "imp2", "imp3", "imp4"},
    },
    ...
    .anomalous_diffusion = {
      .anomalous_diff_id = GKYL_GK_ANOMALOUS_DIFF_D,
      .D_profile = diffusion_D_imp_func,
      .D_profile_ctx = &ctx,
//      .write_diagnostics = true,
    },	
    ...
    .num_diag_moments = 7,
    .diag_moments = { GKYL_F_MOMENT_MAXWELLIAN,GKYL_F_MOMENT_M1, GKYL_F_MOMENT_M2PAR, GKYL_F_MOMENT_M2PERP, GKYL_F_MOMENT_M3PAR, GKYL_F_MOMENT_M3PERP, GKYL_F_MOMENT_BIMAXWELLIAN },
.boundary_flux_diagnostics = {
      .num_diag_moments = 3,
      .diag_moments = { GKYL_F_MOMENT_HAMILTONIAN, GKYL_F_MOMENT_M3PAR, GKYL_F_MOMENT_M3PERP, },
      .num_integrated_diag_moments = 1,
...
//      .time_integrated = true,
  ...
  struct gkyl_gyrokinetic_species imp2 = {
    .name = "imp2",
    .vdim = ctx.vdim,
    .charge = 2*ctx.charge_ion, .mass = ctx.mimp,
    .lower = { ctx.vpar_min_ion_c, ctx.mu_min_ion_c},
    .upper = { ctx.vpar_max_ion_c, ctx.mu_max_ion_c}, 
    .cells = { cells_v[0], cells_v[1] },
    .polarization_density = ctx.n0imp,
    ...
    .collisions =  {
      ...
      //.self_nu = evalNuIon,
      .num_cross_collisions = 5,
      .collide_with = {"elc", "ion", "imp1", "imp3", "imp4"},
    },
...
//      .time_integrated = true,
  ...
  struct gkyl_gyrokinetic_species imp3 = {
    .name = "imp3",
    .vdim = ctx.vdim,
    .charge = 3*ctx.charge_ion, .mass = ctx.mimp,
    .lower = { ctx.vpar_min_ion_c, ctx.mu_min_ion_c},
    .upper = { ctx.vpar_max_ion_c, ctx.mu_max_ion_c}, 
    .cells = { cells_v[0], cells_v[1] },
    .polarization_density = ctx.n0imp,
    ...
    .collisions =  {
      ...
      //.self_nu = evalNuIon,
      .num_cross_collisions = 5,
      .collide_with = {"elc", "ion", "imp1", "imp2", "imp4"},
    },
...
//      .time_integrated = true,
  ...
  struct gkyl_gyrokinetic_species imp4 = {
    .name = "imp4",
    .vdim = ctx.vdim,
    .charge = 4*ctx.charge_ion, .mass = ctx.mimp,
    .lower = { ctx.vpar_min_ion_c, ctx.mu_min_ion_c},
    .upper = { ctx.vpar_max_ion_c, ctx.mu_max_ion_c}, 
    .cells = { cells_v[0], cells_v[1] },
    .polarization_density = ctx.n0imp,
    ...
    .collisions =  {
      ...
      //.self_nu = evalNuIon,
      .num_cross_collisions = 5,
      .collide_with = {"elc", "ion", "imp1", "imp2", "imp3"},
    },
...
//      .time_integrated = true,
  ...
  struct gkyl_efit_inp efit_inp = {
    ...
    .filepath = "./aug36190.geqdsk", // Equilibrium to use.
...
//    .enforce_positivity = true,
    ...
    .num_species = 6,
    .species = { elc, ion, imp1, imp2, imp3, imp4 },
    .field = field,
