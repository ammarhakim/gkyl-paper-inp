// Parallel-propagating firehose instability, with the local closure, for the 10-moment equations.
// Initial proton plasma beta (beta), proton pressure anisotropy (dbeta), and spatial resolution (Nx) were varied,
// but input parameters presented here match the case shown in Fig. 1 from the article:
// J. Walters, K. G. Klein, E. Lichko, J. Juno, and J. TenBarge (2024),
// "Electron Influence on the Parallel Proton Firehose Instability in 10-moment, Multifluid Simulations",
// The Astrophysical Journal, Volume 975(2): 290.
// https://iopscience.iop.org/article/10.3847/1538-4357/ad7c47
...
create_ctx(void)
{
  ...
  // Simulation parameters.
  int Nx = 4480; // Cell count (x-direction).
  double Lx = 12845.57; // Domain size (x-direction).
  double k0_elc = 0.1; // Closure parameter for electrons.
  double k0_ion = 2.743143708051263e-06; // Closure parameter for ions.
  double cfl_frac = 1.0; // CFL coefficient.

  double t_end = 44064000.0; // Final simulation time.
  int num_frames = 300; // Number of output frames.
