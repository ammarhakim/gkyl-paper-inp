...
--local basis = Basis.CartModalTensor { ndim = grid:ndim(), polyOrder = polyOrder }
...
local fldDoFunc = function(t, xn)
   local x, y = xn[1], xn[2]
   local muX, muY   = gauss_mu[1], gauss_mu[2]
   local sigX, sigY = gauss_sig[1], gauss_sig[2]
...
function fileName(varNm)
   return string.format(varNm .. "_Nx%dNy%dP%dTensor_yShP%d_Pos.bp",grid:numCells(1),grid:numCells(2),polyOrder,yShiftPolyOrder)
end
