...
local maxwellian = function(vx, vy, vz, n, ux, uy, uz, vth)
    return n / ((math.sqrt(2 * math.pi * vth ^ 2)) ^ 3) *
               math.exp(-((vx - ux) ^ 2 + (vy - uy) ^ 2 + (vz - uz) ^ 2) / (2 * vth ^ 2))
end

local dy = (2 * Ly) / numCells
local dx = (2 * Lx) / numCells
...
--                 local x = xn[1]
--                 local y = xn[2]
--                 local numDens = n0 / 2 * math.tanh(alph * y / Ly) + 3 / 4 * n0
--                 return numDens
--             end,
...
--                 local numDens = n0 / 2 * math.tanh(alph * y / Ly) + 3 / 4 * n0
--                 local Press =
--                     -grav * mass * (n0 / 2 * math.log(math.cosh(alph * y / Ly)) * Ly / alph + 3 / 4 * n0 * y) + 3 / 4 *
--                         n0 * T0
...
--             local numDens = n0 / 2 * math.tanh(alph * y / Ly) + 3 / 4 * n0
--             local Press =
--                 -grav * mass * (n0 / 2 * math.log(math.cosh(alph * y / Ly)) * Ly / alph + 3 / 4 * n0 * y) + 3 / 4 * n0 *
--                     T0
--             local temp = Press / (numDens)
