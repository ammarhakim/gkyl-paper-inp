...
double polynomial1(double x)
{
  return -1.779197469623974585e+26*pow(x,5.0) + 9.067571703954552816e+26*pow(x,4.0) -1.848464795149837671e+27*pow(x,3.0) + 1.884057445977897785e+27*pow(x,2.0) -9.601506086644861683e+26*x + 1.957205483983441031e+26;	
}

double polynomial2(double x)
{
  return -2.436421088764274825e+27*pow(x,5.0) + 1.275343971246647463e+28*pow(x,4.0) -2.670191079745759084e+28*pow(x,3.0) + 2.795165143814881370e+28*pow(x,2.0) -1.462927573274888568e+28*x + 3.062516932483038385e+27;
}
...
eval_density(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double rho_min = 1.00, rho_turn = 1.04, rho_max = 1.06;
  double x_min = 0.154, x_max = 0.170;
  double rho = (x-x_min)/(x_max-x_min)*(rho_max-rho_min) + rho_min;

  if (rho <= rho_turn)
    fout[0] = polynomial1(rho);
  else
    fout[0] = polynomial2(rho) + 1.0428345521432166e+17;
}
...
eval_temp_elc(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double rho_min = 1.00, rho_turn = 1.03, rho_max = 1.06;
  double x_min = 0.154, x_max = 0.170;
  double rho = (x-x_min)/(x_max-x_min)*(rho_max-rho_min) + rho_min;

  if (rho <= rho_turn)
    fout[0] = (-45*tanh((rho-rho_min)/0.008) + 62.5) * eV;
  else
    fout[0] = (12*tanh((x_max-x)/(x_max-x_min)*(rho_max-rho_min)/0.012) + 5.711832032857547) * eV;
}
...
eval_temp_ion(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  if (rho <= rho_turn)
    fout[0] = (-66*tanh((rho-rho_min)/0.01) + 94.5) * eV;
  else
    fout[0] = (14*tanh((x_max-x)/(x_max-x_min)*(rho_max-rho_min)/0.011) + 15.0569909150555) * eV;
}
...
create_ctx(void)
{
  ...
  double n0 = 1.4e19; // Particle density in 1/m^3
  ...
  double Lx = 0.170 - 0.154;
  double Lz = (M_PI-1.0e-14)*2;

  // Source parameters.
  double x_source = 0.1574;   
  ...
  int Nx = 32; // Number of cells in x.
  int Nz = 96; // Number of cells in z, originally 8.
...
main(int argc, char **argv)
{
  ...
  // GK app
  struct gkyl_gk gk = {
    .name = "gk_bgk_im_asdex_IC2_Nz96_2x2v_p1",

    .cdim = ctx.cdim, .vdim = ctx.vdim,
    .lower = { 0.154, -ctx.Lz/2.0 },   // Originally 0.16
    .upper = { 0.170, ctx.Lz/2.0 },   // Originally 0.175
