...
local function initDistF(A, delta, kv, beta, sigma, v)
   ...
   return A*(1.+delta*math.cos(kv*vMag))*math.exp(-((v[1]-beta[1])^2)/(2.*(sigma^2)))
end
...
local vDim       = #betae
local lambdaMax  = 4.*nuElcIon*((polyOrder+1)^2)*(vte^2)*(1./(dv^2))*vDim*2.2  -- An extra factor of >2 is needed for some reason.
