...
local betaG = 0.      -- Beta parameter in Greene.
...
local function initDistF(A, delta, kv, beta, sigma, v)
   ...
   return A*(1.+delta*math.cos(kv*vMag))*(1./((2.*math.pi*sigma^2)^(1./2.)))
         *math.exp(-((v[1]-beta[1])^2+(v[2]-beta[2])^2+(v[3]-beta[3])^2)/(2.*(sigma^2)))
end
...
local function alpha_E(qs, qr, ms, mr, ns, nr, vts, vtr)
   local logLambda = coulombLog_sr(qs, qr, ms, mr, ns, nr, vts, vtr)  -- Coulomb Logarithm.
...
local dtMin = 2.e-12  -- Approx. dt needed in higher res sim (128x128x128 p=2).
...
vlasovApp = Plasma.App {
   ...
   restartFrameEvery = 0.025,   -- Frequency of outputting restart frame.
   ...
   ion = Plasma.Species {
      charge = qi, mass = mi,
      -- Velocity space grid.
      lower = {vMini, vMini, vMini},
      upper = {vMaxi, vMaxi, vMaxi},
      cells = {Nv, Nv, Nv},
   ...
   elc = Plasma.Species {
      charge = qe, mass = me,
      -- Velocity space grid.
      lower = {vMine, vMine, vMine},
      upper = {vMaxe, vMaxe, vMaxe},
      cells = {Nv, Nv, Nv},
      init = function(t, xn)    -- Initial conditions.
	 x, v = xn[1], {xn[2], xn[3], xn[4]}
