...
--For shafranov shift
a_shift = 0.2
a_mid = R_axis/a_shift - math.sqrt(R_axis*(R_axis - 2*a_shift*R_LCFSmid + 2*a_shift*R_axis))/a_shift

-- What are these values??
qAxis      = 0.7
qSep       = 3.0              -- Safety factor at the separatrix.
--sSep       = 1.27976219        -- Magnetic shear at the separatrix.
kappa      = 1.6 -- 1.33              -- Elongation (=1 for no elongation).
...
local function R(r, theta) return R_axis - a_shift*r^2/(2*R_axis) + r*math.cos(theta + math.asin(delta)*math.sin(theta)) end
