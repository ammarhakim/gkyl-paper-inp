...
delta      = 0.           -- Triangularity (=0 for no triangularity).
...
function s(r) return 2*r^2/(qprofile(r)*a_mid)^2 * (qSep - qAxis) end
...
local function R(r, theta) return R_axis + r*math.cos(theta) + r*(kappa - 1)/(kappa + 1)
      *(qprofile(r)*s(r) + 6*qAxis)/(qSep*s(a_mid) + 6*qAxis)*math.cos(theta)
      + delta*r^2/(4*a_mid)*(qprofile(r)*s(r) + 4*qAxis)/(qSep*s(a_mid) + 4*qAxis)*math.cos(2*theta) end
local function Z(r, theta) return r*math.sin(theta) - r*(kappa - 1)/(kappa + 1)
      *(qprofile(r)*s(r) + 6*qAxis)/(qSep*s(a_mid) + 6*qAxis)*math.sin(theta)
      - delta*r^2/(4*a_mid)*(qprofile(r)*s(r) + 4*qAxis)/(qSep*s(a_mid) + 4*qAxis)*math.sin(2*theta) end
