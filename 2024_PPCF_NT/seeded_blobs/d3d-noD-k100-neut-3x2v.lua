...
delta      = 0.0        -- Triangularity (=0 for no triangularity).
...
plasmaApp = Plasma.App {
   ...
   decompCuts = {16,12,4},    -- MPI subdomains/processes.
   ...
   ion = Plasma.Species {
      ...
      init = Plasma.VmMaxwellianProjection {
	    ...
	    -- end
	    if z <= 0 then
	       return n_n*(sech((-Lz/2-z)/z0)^2) + flr +
		  n0*(sech((xmax-x)/x0)^2)*math.exp(-z^2/(2*.62^2))
	    else                
	       return n_n*(sech((Lz/2-z)/z0)^2) + flr +
		  n0*(sech((xmax-x)/x0)^2)*math.exp(-z^2/(2*.62^2))
	    end
