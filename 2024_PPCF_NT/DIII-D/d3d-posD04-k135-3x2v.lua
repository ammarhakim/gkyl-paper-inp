...
-- parameters similar to DIII-D IWL shot 171650
--
-- Simulation used gkyl changeset 6a5d402fda46.
--
-------------------------------------------------------------------------------
...
R_axisTrue = 1.6486461 -- [m]  -- Change R_axis to fit geometry better.
R_axis     = 1.6 -- [m]
B_axis     = 2*R_axisTrue/R_axis -- [T]
R_LCFSmid  = 2.17                     -- Major radius of the LCFS at the outboard midplane [m].
x_in       = 0.1                      -- x-domain inside LCFS.
x_out      = 0.05                     -- x-domain outside LCFS.
...
kappa      = 1.35              -- Elongation (=1 for no elongation).
delta      = 0.4               -- Triangularity (=0 for no triangularity).

function r_x(x) return x+a_mid-x_in end -- Minor radius given x.
function qprofile(r)
