...
create_ctx(void)
{
  ...
  double alpha = 1.0e-9; // Applied perturbation amplitude.
  double kx = 0.4; // Perturbed wave number (x-direction).

  // Derived physical quantities (using normalized code units).
  double gamma_elc1 = 1.0 / sqrt(1.0 - (ux_elc1 * ux_elc1) - (uy_elc1 * uy_elc1) ); // First electron gamma factor.
  double gamma_elc2 = 1.0 / sqrt(1.0 - (ux_elc2 * ux_elc2) - (uy_elc2 * uy_elc2) ); // Second electron gamma factor.
  ...
  int Nvy = 48; // Cell count (velocity space: vy-direction).
  ...
  double t_end = 45.0; // Final simulation time.
