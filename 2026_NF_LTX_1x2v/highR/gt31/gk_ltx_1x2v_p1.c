...
void density_init(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double nPeak   = 65.*sqrt(5.)/3./c_ss*Ls*effSrc/2.;
  double perturb = 0.;
  if (fabs(z) <= Ls) {
...
void nu_elc_profile(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_app_ctx *app = ctx;
  fout[0] = app->nuElc;
}
void nu_ion_profile(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct gk_app_ctx *app = ctx;
  fout[0] = app->nuIon;
}
...
create_ctx(void)
{
  ...
  // Geometry and magnetic field.
  double B0 = 0.5*(4.105327e-01+1.463064e-01); // Average bmag in sim with geometry.
  double Lz = 6.283196444907266;  // Field line length of sim with geometry.
  ...
  // Source parameters
  double n_src = 1.65e22;
  double Te_src = 114*eV;
  double Ti_src = 12*eV;
  ...
  // Physical velocity space limits
  double vpar_max_elc = 4.0*vte;
  double mu_max_elc = me*pow(1.5*4.0*vte,2)/(2.0*B0);

  double vpar_max_ion = 4.0*vti;
  double mu_max_ion = mi*pow(1.5*4.0*vti,2)/(2.0*B0);
