...
phaseDecomp = DecompRegionCalc4D.CartProd { cuts = {64,1,1,1} }
...
phaseGridElc = Grid.RectCart4D {
   lower = {XL, VL_ELC, VL_ELC, VL_ELC},
   upper = {XU, VU_ELC, VU_ELC, VU_ELC},
   cells = {NX, NV, NV, NV},
   decomposition = phaseDecomp,  
}
-- phase space grid for ions
phaseGridIon = Grid.RectCart4D {
   lower = {XL, VL_ION, VL_ION, VL_ION},
   upper = {XU, VU_ION, VU_ION, VU_ION},
   cells = {NX, NV, NV, NV},
   decomposition = phaseDecomp,
}
...
distfMaxwellElc = DataStruct.Field4D {
   onGrid = phaseGridElc,
   numComponents = phaseBasisElc:numNodes(),
   ghost = {1, 1},
}
distfBgkElc = DataStruct.Field4D {
   onGrid = phaseGridElc,
   numComponents = phaseBasisElc:numNodes(),
   ghost = {1, 1},
}
...
distfTempElc = DataStruct.Field4D {
   onGrid = phaseGridElc,
   numComponents = phaseBasisElc:numNodes(),
   ghost = {1, 1},
}
distfMaxwellIon = DataStruct.Field4D {
   onGrid = phaseGridIon,
   numComponents = phaseBasisIon:numNodes(),
   ghost = {1, 1},
}
distfBgkIon = DataStruct.Field4D {
   onGrid = phaseGridIon,
   numComponents = phaseBasisIon:numNodes(),
   ghost = {1, 1},
}
...
distfTempIon = DataStruct.Field4D {
   onGrid = phaseGridIon,
   numComponents = phaseBasisIon:numNodes(),
   ghost = {1, 1},
}
...
distfIonizationElc = DataStruct.Field4D {
   onGrid = phaseGridElc,
   numComponents = phaseBasisElc:numNodes(),
   ghost = {1, 1},
}
distfIonizationIon = DataStruct.Field4D {
   onGrid = phaseGridIon,
   numComponents = phaseBasisIon:numNodes(),
   ghost = {1, 1},
}
...
ptclEnergyElc = DataStruct.Field1D {
   onGrid = confGrid,
   numComponents = 6*confBasis:numNodes(),
   ghost = {1, 1},
}
-- Ion particle energy
...
initProjectFluxElc = Updater.ProjectOnNodalBasis1D {
   ...
   evaluate = function (x,y,z,t)
      local idx = round(2*math.abs(x))
      if x>= 0 then
	 return loadedFluxElc[idx], 0, 0
      else
	 return -1*loadedFluxElc[idx], 0, 0
      end
   end
}
initProjectFluxIon = Updater.ProjectOnNodalBasis1D {
   ...
   evaluate = function (x,y,z,t)
      local idx = round(2*math.abs(x))
      if x >= 0 then
	 return loadedFluxIon[idx], 0, 0
      else
	 return -1*loadedFluxIon[idx], 0, 0
      end
   end
}
...
initProjectPtclEnergyElc = Updater.ProjectOnNodalBasis1D {
   ...
   evaluate = function (x,y,z,t)
	 ...
	 loadedFluxElc[idx]*loadedFluxElc[idx]/loadedNumDensityElc[idx], vtElc^2*loadedNumDensityElc[idx], vtElc^2*loadedNumDensityElc[idx]
   end
}
initProjectPtclEnergyIon = Updater.ProjectOnNodalBasis1D {
   ...
   evaluate = function (x,y,z,t)
      ...
      return vtIon^2*loadedNumDensityIon[idx] + 
        loadedFluxIon[idx]*loadedFluxIon[idx]/loadedNumDensityIon[idx], vtIon^2*loadedNumDensityIon[idx], vtIon^2*loadedNumDensityIon[idx]
   end
}

-- updaters to initialize Maxwellian distribution from moments
initMaxwellianElc = Updater.MaxwellDistInit1X3V {
   onGrid = phaseGridElc,
   phaseBasis = phaseBasisElc,
   confBasis = confBasis,
}
initMaxwellianIon = Updater.MaxwellDistInit1X3V {
   onGrid = phaseGridIon,
...
-- BGK RHS updaters
bgkCollElc = Updater.BGKCollUpdater1X3V {
   onGrid = phaseGridElc,
   phaseBasis = phaseBasisElc,
   confBasis = confBasis,
   mass = elcMass,
   elemCharge = elcCharge,
   permitivity = epsilon0,
}
bgkCollIon = Updater.BGKCollUpdater1X3V {
   onGrid = phaseGridIon,
   phaseBasis = phaseBasisIon,
   confBasis = confBasis,
   mass = ionMass,
...
-- operator
initIonizationDistfElc = Updater.MaxwellDistInit1X3V {
   onGrid = phaseGridElc,
   phaseBasis = phaseBasisElc,
   confBasis = confBasis,
   zeroDriftOutput = true,
}
initIonizationDistfIon = Updater.MaxwellDistInit1X3V {
   onGrid = phaseGridIon,
   phaseBasis = phaseBasisIon,
   confBasis = confBasis,
   zeroDriftOutput = true,
...
vlasovSolverElc = Updater.EigenNodalVlasov1X3V {
   onGrid = phaseGridElc,
   phaseBasis = phaseBasisElc,
   confBasis = confBasis,
   cfl = cfl,
...
vlasovSolverIon = Updater.EigenNodalVlasov1X3V {
   onGrid = phaseGridIon,
   phaseBasis = phaseBasisIon,
   confBasis = confBasis,   
   cfl = cfl,
...
fluxCalcElc = Updater.DistFuncMomentCalc1X3V {
   onGrid = phaseGridElc,
   phaseBasis = phaseBasisElc,
   confBasis = confBasis,
   moment = 1,
}
fluxCalcIon = Updater.DistFuncMomentCalc1X3V {
   onGrid = phaseGridIon,
   phaseBasis = phaseBasisIon,
   confBasis = confBasis,
   moment = 1,
...
ptclEnergyCalcElc = Updater.DistFuncMomentCalc1X3V {
   onGrid = phaseGridElc,
   phaseBasis = phaseBasisElc,
   confBasis = confBasis,
   moment = 2,
}
ptclEnergyCalcIon = Updater.DistFuncMomentCalc1X3V {
   onGrid = phaseGridIon,
   phaseBasis = phaseBasisIon,
   confBasis = confBasis,
   moment = 2,
...
-- BC updaters
setDistfElcBCLower = Updater.Bc4D {
   onGrid = phaseGridElc,
...
setDistfElcBCUpper = Updater.Bc4D {
   onGrid = phaseGridElc,
   boundaryConditions = {bcDistfZero},
   dir = 0,
   edge = "upper",
}
setDistfIonBCLower = Updater.Bc4D {
   onGrid = phaseGridIon,
   --boundaryConditions = {bcFunctionElc},
   boundaryConditions = {bcDistfZero},
   dir = 0,
...
setDistfIonBCUpper = Updater.Bc4D {
   onGrid = phaseGridIon,
   boundaryConditions = {bcDistfZero},
   dir = 0,
   edge = "upper",
