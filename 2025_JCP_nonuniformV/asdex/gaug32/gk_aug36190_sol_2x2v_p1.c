...
create_ctx(void)
{
  ...
  // Physical velocity space limits
  double vpar_max_elc = 3.0*vt_elc;
  double mu_max_elc = me*pow(4.0*vt_elc,2)/(2.0*B0);

  double vpar_max_ion = 3.0*vt_ion;
  ...
  int Nmu = 64; // Number of cells in mu.

  double t_end = 2*4*5*100.0e-6;
  double num_frames = 2*4*5*50;
  double write_phase_freq = 0.4; // Frequency of writing phase-space diagnostics (as a fraction of num_frames).
